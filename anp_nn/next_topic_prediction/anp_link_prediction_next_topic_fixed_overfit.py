...
# Replace SAGEConv with GATConv
class GNNEncoder(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels):
        super().__init__()
        self.conv1 = GATConv((-1, -1), hidden_channels, heads=4, concat=True, add_self_loops=False)  # Disattiva self-loops
        self.bn1 = BatchNorm1d(hidden_channels * 4)  # * 4 per il numero di teste dell'attenzione
...
class EdgeDecoder(torch.nn.Module):
    def __init__(self, hidden_channels):
        super().__init__()
        # Aggiorna la dimensione di Linear per accettare 256 come input
        self.lin1 = Linear(2 * hidden_channels * 4, hidden_channels)  # 2 * 32 * 4 = 256
        self.lin2 = Linear(hidden_channels, 1)

    def forward(self, z_dict, edge_label_index):
        row, col = edge_label_index
        # Concatenazione delle embedding di author e topic
        z = torch.cat([z_dict['author'][row], z_dict['topic'][col]], dim=-1)
        # Verifica della dimensione
        z = self.lin1(z).relu()  # Output: [batch_size, 32]
        z = self.lin2(z)          # Output: [batch_size, 1]
        return z.view(-1)
...
optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=0.01)  # aggiungo weight decay
